import { validateIsoDate, type IsoDate } from '../date';
...
describe('validateIsoDate', () => {
    ...
    describe('invalid format', () => {
        it('should throw error for date with wrong separator', () => {
            expect(() => validateIsoDate('2024/01/15')).toThrow(
                "Invalid date '2024/01/15'. Expected YYYY-MM-DD."
            );
        });

        it('should throw error for date without separators', () => {
            expect(() => validateIsoDate('20240115')).toThrow(
                "Invalid date '20240115'. Expected YYYY-MM-DD."
            );
        });

        it('should throw error for date with time component', () => {
            expect(() => validateIsoDate('2024-01-15T12:00:00')).toThrow(
                "Invalid date '2024-01-15T12:00:00'. Expected YYYY-MM-DD."
            );
        });

        it('should throw error for short year format', () => {
            expect(() => validateIsoDate('24-01-15')).toThrow(
                "Invalid date '24-01-15'. Expected YYYY-MM-DD."
            );
        });

        it('should throw error for single digit month', () => {
            expect(() => validateIsoDate('2024-1-15')).toThrow(
                "Invalid date '2024-1-15'. Expected YYYY-MM-DD."
            );
        });

        it('should throw error for single digit day', () => {
            expect(() => validateIsoDate('2024-01-5')).toThrow(
                "Invalid date '2024-01-5'. Expected YYYY-MM-DD."
            );
        });

        it('should throw error for reversed date format (DD-MM-YYYY)', () => {
            expect(() => validateIsoDate('15-01-2024')).toThrow(
                "Invalid date '15-01-2024'. Expected YYYY-MM-DD."
            );
        });

        it('should throw error for US format (MM-DD-YYYY)', () => {
            expect(() => validateIsoDate('01-15-2024')).toThrow(
                "Invalid date '01-15-2024'. Expected YYYY-MM-DD."
            );
        });
    });
    ...
    describe('invalid date values', () => {
        it('should throw error for invalid month', () => {
            expect(() => validateIsoDate('2024-13-01')).toThrow(
                "Invalid date '2024-13-01'."
            );
        });

        it('should throw error for zero month', () => {
            expect(() => validateIsoDate('2024-00-15')).toThrow(
                "Invalid date '2024-00-15'."
            );
        });

        it('should throw error for invalid day', () => {
            expect(() => validateIsoDate('2024-01-32')).toThrow(
                "Invalid date '2024-01-32'."
            );
        });

        it('should throw error for zero day', () => {
            expect(() => validateIsoDate('2024-01-00')).toThrow(
                "Invalid date '2024-01-00'."
            );
        });
    ...
    describe('edge cases', () => {
        ...
        it('should throw error for null-like strings', () => {
            expect(() => validateIsoDate('null')).toThrow(
                "Invalid date 'null'. Expected YYYY-MM-DD."
            );
        });

        it('should throw error for undefined-like strings', () => {
            expect(() => validateIsoDate('undefined')).toThrow(
                "Invalid date 'undefined'. Expected YYYY-MM-DD."
            );
        });
    ...
    describe('type compatibility', () => {
        it('should return IsoDate type', () => {
            const result: IsoDate = validateIsoDate('2024-01-15');
            expect(typeof result).toBe('string');
        });
