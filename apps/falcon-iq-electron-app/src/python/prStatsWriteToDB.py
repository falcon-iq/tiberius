    ...
    for filename in status_files:
        ...
        if file_path.exists():
            try:
                ...
                updated_count += 1
                if not quiet:
                    print(f"   ✅ Marked completed: {filename}")
            except Exception as e:
...
def import_all_pr_stats(db_path: Path, base_dir: Path, pr_data_folder: Path, task_folder: Path, 
    ...
    if not stats_folder.exists():
        if not quiet:
            print(f"❌ Stats folder not found: {stats_folder}")
    ...
    try:
        ...
        for user in users:
            ...
            if not task_file:
                if not quiet:
                    print(f"   ⚠️  No task files found for user")
                continue
                ...
                if result['errors'] == 0:
                    ...
                    if delete_after_import:
                        try:
                            ...
                            total_deleted += 1
                            if not quiet:
                                print(f"   🗑️  Deleted: {csv_filename}")
                        except Exception as e:
                            if not quiet:
                                print(f"   ⚠️  Failed to delete {csv_filename}: {e}")
                    
                    # Clean up intermediate status files and mark main status as completed
                    if not quiet:
                        print(f"   🧹 Cleaning up status files...")
                    ...
                    if not quiet and cleanup_result['errors'] > 0:
                        print(f"   ⚠️  Cleanup had {cleanup_result['errors']} error(s)")
...
def main():
    ...
    print(f"🗑️  Delete after import: {not args.keep_files}")
    print()
