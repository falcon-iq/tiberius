...
import { initDatabase, getUsers, addUser, deleteUser, getGoals, addGoal, deleteGoal, updateGoal, getPRCommentStats, getPRCommentsByMetric, closeDatabase, type AddUserInput, type AddGoalInput, type UpdateGoalInput } from './database';
...
ipcMain.handle('db:getPRCommentsByMetric', (_event, username: string, metricType: string) =>
  getPRCommentsByMetric(username, metricType as any)
);
...
// IPC handlers for Python server data endpoints
ipcMain.handle('python:getPR', async (_event, prId: number) => {
  try {
    const response = await fetch(`http://localhost:8765/api/pr/${prId}`);
    if (!response.ok) {
...
ipcMain.handle('python:getComment', async (_event, prId: number, commentId: number) => {
  try {
    const response = await fetch(`http://localhost:8765/api/pr/${prId}/comment/${commentId}`);
    if (!response.ok) {
      return { success: false, error: `HTTP ${response.status}` };
    }
    const data = await response.json();
    ...
    return { success: false, error: 'Failed to fetch comment details' };
  }
});

ipcMain.handle('python:getPRFiles', async (_event, prId: number) => {
  try {
    const response = await fetch(`http://localhost:8765/api/pr/${prId}/files`);
    if (!response.ok) {
      return { success: false, error: `HTTP ${response.status}` };
    }
    const data = await response.json();
