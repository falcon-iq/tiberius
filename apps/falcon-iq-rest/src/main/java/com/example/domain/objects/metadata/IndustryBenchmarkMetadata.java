...
public class IndustryBenchmarkMetadata implements GenericBeanMetadata {
    ...
    public static final GenericBeanFieldMetadata<Long> GENERATED_AT = GenericBeanFieldMetadataBuilder.<Long>builder()
            .name("generatedAt").label("Generated At").type(FieldType.EPOCH_MILLISECOND).updateAllowed(false)
            .required(false).sortSupported(true).filterSupported(true).build();

    public static final GenericBeanFieldMetadata<List> COMPANIES = GenericBeanFieldMetadataBuilder.<List>builder()
            .name("companies").label("Companies").type(FieldType.LIST).updateAllowed(false)
            .required(false).sortSupported(false).filterSupported(false).build();

    public static final GenericBeanFieldMetadata<String> ERROR_MESSAGE = GenericBeanFieldMetadataBuilder.<String>builder()
            .name("errorMessage").label("Error Message").type(FieldType.STRING).updateAllowed(true)
            .required(false).sortSupported(false).filterSupported(false).build();

    @Override
    public List<GenericBeanFieldMetadata<?>> getFields() {
        return List.of(ID, CREATED_AT, MODIFIED_AT, INDUSTRY_NAME, COUNTRY, SLUG, STATUS, GENERATED_AT, COMPANIES, ERROR_MESSAGE);
    }
}
